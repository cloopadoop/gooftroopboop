...
CapcomSnesInstrSet::CapcomSnesInstrSet(RawFile *file, uint32_t offset, uint32_t spcDirAddr, const std::string &name) :
    GTBInstrSet(CapcomSnesFormat::name, file, offset, 0, name),
...
CapcomSnesInstr::CapcomSnesInstr(GTBInstrSet *instrSet,
                                 uint32_t offset,
                                 uint32_t theBank,
                                 uint32_t theInstrNum,
                                 uint32_t spcDirAddr,
                                 const std::string &name) :
    GTBInstr(instrSet, offset, 6, theBank, theInstrNum, name),
    spcDirAddr(spcDirAddr) {
}
...
CapcomSnesRgn::CapcomSnesRgn(CapcomSnesInstr *instr, uint32_t offset) :
  ...
  snesConvADSR<GTBRgn>(this, adsr1, adsr2, gain);
