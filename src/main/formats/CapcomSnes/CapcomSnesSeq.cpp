...
CapcomSnesSeq::CapcomSnesSeq(RawFile *file,
                             ...
                             std::string name)
    : GTBSeq(CapcomSnesFormat::name, file, seqdataOffset, 0, std::move(name)), version(ver),
...
bool CapcomSnesSeq::parseHeader() {
  ...
  GTBHeader *seqHeader = addHeader(dwOffset, (priorityInHeader ? 1 : 0) + MAX_TRACKS * 2, "Sequence Header");
