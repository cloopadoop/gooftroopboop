...
class SNESSampColl
 ...
 public:
  ...
  SNESSampColl(const std::string& format, GTBInstrSet* instrset, uint32_t offset, uint32_t maxNumSamps = 256);
  ...
  SNESSampColl(const std::string& format, GTBInstrSet* instrset, uint32_t offset, const std::vector<uint8_t>& targetSRCNs, std::string name = "SNESSampColl");
 ...
 protected:
  GTBHeader *spcDirHeader{nullptr};
  uint32_t spcDirAddr;
...
class SNESSamp
    : public GTBSamp {
 public:
  SNESSamp(GTBSampColl *sampColl, uint32_t offset, uint32_t length, uint32_t dataOffset,
