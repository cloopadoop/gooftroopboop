...
SNESSampColl::SNESSampColl(const std::string &format, RawFile *rawfile, uint32_t offset, uint32_t maxNumSamps) :
    GTBSampColl(format, rawfile, offset, 0),
    spcDirAddr(offset) {
  setDefaultTargets(maxNumSamps);
}

SNESSampColl::SNESSampColl(const std::string &format, GTBInstrSet *instrset, uint32_t offset, uint32_t maxNumSamps) :
    GTBSampColl(format, instrset->rawFile(), instrset, offset, 0),
    spcDirAddr(offset) {
  setDefaultTargets(maxNumSamps);
}
...
SNESSampColl::SNESSampColl(const std::string &format, RawFile *rawfile, uint32_t offset,
    ...
    GTBSampColl(format, rawfile, offset, 0, std::move(name)),
    spcDirAddr(offset),
    targetSRCNs(targetSRCNs) {
}

SNESSampColl::SNESSampColl(const std::string &format, GTBInstrSet *instrset, uint32_t offset,
                           const std::vector<uint8_t> &targetSRCNs, std::string name) :
    GTBSampColl(format, instrset->rawFile(), instrset, offset, 0, std::move(name)),
    spcDirAddr(offset),
    targetSRCNs(targetSRCNs) {
}
...
SNESSamp::SNESSamp(GTBSampColl *sampColl, uint32_t offset, uint32_t length, uint32_t dataOffset,
                   uint32_t dataLen, uint32_t loopOffset, std::string name)
    : GTBSamp(sampColl, offset, length, dataOffset, dataLen, 1, 16, 32000, std::move(name)),
