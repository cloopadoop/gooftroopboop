...
bool mainDLSCreation(
  ...
  std::vector<GTBSampColl *> finalSampColls;
  ...
  for (size_t inst = 0; inst < m_instrsets.size(); inst++) {
    ...
    for (size_t i = 0; i < nInstrs; i++) {
      ...
      */
      ...
      DLSInstr *newInstr = dls.addInstr(bank_no, GTBinstr->instrNum, name);
      for (uint32_t j = 0; j < nRgns; j++) {
        ...
        GTBSampColl *sampColl = rgn->sampCollPtr;
        if (!sampColl) {
