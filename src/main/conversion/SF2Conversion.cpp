...
SynthFile* createSynthFile(
  const std::vector<GTBInstrSet*>& m_instrsets,
  const std::vector<GTBSampColl*>& m_sampcolls
) {
  ...
  std::vector<GTBSamp *> finalSamps;
  std::vector<const GTBSampColl *> finalSampColls;
  ...
  for (size_t inst = 0; inst < m_instrsets.size(); inst++) {
    const GTBInstrSet* set = m_instrsets[inst];
    size_t nInstrs = set->aInstrs.size();
    for (size_t i = 0; i < nInstrs; i++) {
      GTBInstr* GTBinstr = set->aInstrs[i];
      size_t nRgns = GTBinstr->regions().size();
      if (nRgns == 0)  // do not write an instrument if it has no regions
        continue;
      SynthInstr* newInstr = synthfile->addInstr(GTBinstr->bank, GTBinstr->instrNum, GTBinstr->reverb);
      for (uint32_t j = 0; j < nRgns; j++) {
        GTBRgn* rgn = GTBinstr->regions()[j];
        ...
        const GTBSampColl* sampColl = rgn->sampCollPtr;
        if (!sampColl) {
          // If rgn is of an InstrSet with an embedded SampColl, use that SampColl.
          if (static_cast<GTBInstrSet*>(rgn->gtbFile())->sampColl)
                sampColl = static_cast<GTBInstrSet*>(rgn->gtbFile())->sampColl;
        ...
        if (realSampNum >= finalSamps.size()) {
          L_ERROR("Sample {} does not exist. Instr index: {:d}, Instr num: {:d}, Region index: {:d}", realSampNum, i, GTBinstr->instrNum, j);
          realSampNum = finalSamps.size() - 1;
        }

        GTBSamp* samp = finalSamps[realSampNum];  // sampColl->samples[rgn->sampNum];
...
void unpackSampColl(SynthFile &synthfile, const GTBSampColl *sampColl, std::vector<GTBSamp *> &finalSamps) {
  ...
  for (size_t i = 0; i < nSamples; i++) {
    GTBSamp *samp = sampColl->samples[i];
