...
bool saveAsDLS(GTBInstrSet &set, const std::string &filepath) {
  GTBColl* coll = !set.assocColls.empty() ? set.assocColls.front() : nullptr;
  if (!coll && !set.sampColl)
    return false;

  std::vector<GTBInstrSet*> instrsets;
  std::vector<GTBSampColl*> sampcolls;
  if (coll) {
...
bool saveAsSF2(GTBInstrSet &set, const std::string &filepath) {
  GTBColl* coll = !set.assocColls.empty() ? set.assocColls.front() : nullptr;
  if (!coll && !set.sampColl)
    return false;
...
bool saveAsSF2(const GTBColl &coll, const std::string &filepath) {
...
void saveAllAsWav(const GTBSampColl &coll, const std::string &save_dir) {
...
bool saveAsOriginal(const GTBFile& file, const std::string& filepath) {
