...
uint32_t ParseNumericTextField(const std::string& raw_value, const char* field_name) {
  auto begin = std::find_if_not(raw_value.begin(), raw_value.end(), [](unsigned char c) {
    return std::isspace(c) != 0;
  });
  auto end = std::find_if_not(raw_value.rbegin(), raw_value.rend(), [](unsigned char c) {
    return std::isspace(c) != 0;
  }).base();
  ...
  const bool is_all_digits = std::all_of(begin, end, [](unsigned char c) {
    return std::isdigit(c) != 0;
  });
  if (!is_all_digits) {
    L_WARN("Invalid textual ID666 {} field '{}'; using 0", field_name, raw_value);
    return 0;
  }

  try {
    return static_cast<uint32_t>(std::stoul(std::string(begin, end), nullptr, 10));
  } catch (const std::exception& e) {
    L_WARN("Failed parsing textual ID666 {} field '{}': {}. Using 0", field_name, raw_value, e.what());
    return 0;
  }
}
...
GTBTag SPCFile::tagFromSPCFile(const SPCFile& spc) {
  const auto& id666 = spc.id666Tag();
  const auto& dsp = spc.dsp();

  auto tag = GTBTag(id666.songTitle, id666.artist, id666.gameTitle, id666.comments);
...
void SPCFile::loadID666Tag(const RawFile& file) {
  ...
  // SPC ID666 has two legacy variants:
  //  - 0x1a: textual fields for date/play/fade
  //  - 0x1b: binary fields
  const uint8_t id666_format = m_hasID666Tag;
  if (id666_format == 0x1b) {
    m_id666Tag.dateDumped = std::to_string(file.readWord(0x9E));
    m_id666Tag.secondsToPlay = file.readWord(0xA9);
    m_id666Tag.fadeLength = file.readWord(0xAC);
  } else {
    m_id666Tag.dateDumped = file.readNullTerminatedString(0x9E, 11);
    m_id666Tag.secondsToPlay = ParseNumericTextField(file.readNullTerminatedString(0xA9, 3), "seconds");
    m_id666Tag.fadeLength = ParseNumericTextField(file.readNullTerminatedString(0xAC, 5), "fade");
  }
}
