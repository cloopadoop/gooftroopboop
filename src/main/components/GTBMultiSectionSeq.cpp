...
GTBMultiSectionSeq::GTBMultiSectionSeq(const std::string& format,
                                       RawFile *file,
                                       uint32_t offset,
                                       uint32_t length,
                                       std::string name)
    : GTBSeq(format, file, offset, length, std::move(name)),
      dwStartOffset(offset) {
}

void GTBMultiSectionSeq::resetVars() {
  GTBSeq::resetVars();

  foreverLoops = 0;
}

bool GTBMultiSectionSeq::load() {
...
bool GTBMultiSectionSeq::loadTracks(ReadMode readMode, uint32_t stopTime) {
  ...
  // load all tracks
  uint32_t stopOffset = gtbFile()->endOffset();
...
bool GTBMultiSectionSeq::postLoad() {
...
bool GTBMultiSectionSeq::loadSection(GTBSeqSection *section, uint32_t stopTime) {
  // reset variables
...
bool GTBMultiSectionSeq::isOffsetUsed(uint32_t offset) {
  return isItemAtOffset(offset, false);
}

bool GTBMultiSectionSeq::readEvent(long /*stopTime*/) {
  return false;        //by default, don't add any events, just stop immediately.
}

void GTBMultiSectionSeq::addSection(GTBSeqSection *section) {
...
bool GTBMultiSectionSeq::addLoopForeverNoItem() {
  foreverLoops++;
  if (readMode == READMODE_ADD_TO_UI) {
    return false;
  }
...
GTBSeqSection *GTBMultiSectionSeq::getSectionAtOffset(uint32_t offset) {
  for (size_t sectionIndex = 0; sectionIndex < aSections.size(); sectionIndex++) {
    GTBSeqSection *section = aSections[sectionIndex];
