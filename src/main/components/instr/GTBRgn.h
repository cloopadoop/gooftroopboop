...
class GTBRgn : public GTBItem {
 public:
  GTBRgn(GTBInstr *instr, uint32_t offset, uint32_t length = 0, std::string name = "Region");
  GTBRgn(GTBInstr *instr, uint32_t offset, uint32_t length, uint8_t keyLow, uint8_t keyHigh, uint8_t velLow,
...
public:
  ...
  // sample's offset relative to the beginning of sample data (GTBSampColl::sampDataOffset)

  // For formats that use multiple sampColls and reference samples base 0 for each sampColl (NDS)
  GTBSampColl *sampCollPtr {nullptr};
...
class GTBRgnItem : public GTBItem {
 public:
  enum RgnItemType {
    RIT_GENERIC,
    RIT_UNKNOWN,
  ...
  GTBRgnItem(const GTBRgn *rgn, RgnItemType theType, uint32_t offset, uint32_t length, std::string name);
