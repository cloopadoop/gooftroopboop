...
GTBSamp::GTBSamp(GTBSampColl *sampColl, uint32_t offset, uint32_t length, uint32_t dataOffset,
                 uint32_t dataLen, uint8_t nChannels, uint16_t bps, uint32_t rate,
                 std::string name)
    : GTBItem(sampColl->gtbFile(), offset, length, std::move(name), Type::Sample),
      dataOff(dataOffset), dataLength(dataLen), bps(bps), rate(rate), channels(nChannels),
      parSampColl(sampColl) {
}

double GTBSamp::compressionRatio() {
  return 1.0;
}

void GTBSamp::convertToStdWave(std::uint8_t* buf) {
...
void GTBSamp::setVolume(double volume) {
  m_attenDb = ampToDb(volume);
}

bool GTBSamp::onSaveAsWav() {
...
bool GTBSamp::saveAsWav(const std::string &filepath) {
  uint32_t bufSize;
