...
class GTBSamp : public GTBItem {
public:
  GTBSamp(GTBSampColl *sampColl, uint32_t offset = 0, uint32_t length = 0, uint32_t dataOffset = 0,
          uint32_t dataLength = 0, uint8_t channels = 1, uint16_t bps = 16, uint32_t rate = 0,
          std::string name = "Sample");
  ~GTBSamp() override = default;
...
class EmptySamp : public GTBSamp {
public:
  EmptySamp(GTBSampColl* sampColl): GTBSamp(sampColl, 0, 0, 0, 16) {}
