...
bool FilegroupMatcher::onNewSeq(GTBSeq *seq) {
  seqs.push_back(seq);
  return true;
}

bool FilegroupMatcher::onNewInstrSet(GTBInstrSet *instrset) {
  instrsets.push_back(instrset);
  return true;
}

bool FilegroupMatcher::onNewSampColl(GTBSampColl *sampcoll) {
  sampcolls.push_back(sampcoll);
  return true;
}

bool FilegroupMatcher::onCloseSeq(GTBSeq *seq) {
...
bool FilegroupMatcher::onCloseInstrSet(GTBInstrSet *instrset) {
...
bool FilegroupMatcher::onCloseSampColl(GTBSampColl *sampcoll) {
...
void FilegroupMatcher::lookForMatch() {
  ...
  // An instrument set plus an optional extra sample collection to add to the GTBColl
  struct InstrAssoc {
    GTBInstrSet* instrSet;
  ...
  GTBSampColl* lastValidSamp = nullptr;

  // 2. Build the list of usable (instrset, extraSamp) associations
  for (GTBInstrSet* instr : instrsets) {
    GTBSampColl* extraSamp = nullptr;
  ...
  // 3. For every sequence, build a GTBColl with the next instrument association
