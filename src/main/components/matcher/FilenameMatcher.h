...
public:
  ...
  bool seqId(GTBSeq *seq, std::string &id) override {
    RawFile *rawfile = seq->rawFile();
    id = rawfile->path().string();
    return !id.empty();
  }

  bool instrSetId(GTBInstrSet *instrset, std::string &id) override {
    RawFile *rawfile = instrset->rawFile();
    id = rawfile->path().string();
    return !id.empty();
  }

  bool sampCollId(GTBSampColl *sampcoll, std::string &id) override {
