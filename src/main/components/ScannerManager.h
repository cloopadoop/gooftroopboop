...
using scannerSpawner = std::function<std::shared_ptr<GTBScanner>()>;
...
class ScannerManager final {
 public:
  ...
  std::vector<std::shared_ptr<GTBScanner>> scanners() const {
    std::vector<std::shared_ptr<GTBScanner>> tmp(m_generators.size());
    std::ranges::transform(m_generators, tmp.begin(), [](auto pair) { return pair.second(); });

    return tmp;
  }

  std::vector<std::shared_ptr<GTBScanner>> scannersWithExtension(const std::string& ext) const {
    std::vector<std::shared_ptr<GTBScanner>> tmp;
...
namespace GTBoop::scanners {
template <typename T>
class ScannerRegistration final {
 public:
  explicit ScannerRegistration(const char* formatName) {
    ScannerManager::get().add(formatName, [formatName]() -> std::shared_ptr<GTBScanner> {
  ...
  ScannerRegistration(const char* formatName, std::initializer_list<const char*> exts) {
    ...
    sm.add(formatName, [formatName]() -> std::shared_ptr<GTBScanner> {
      auto format = Format::formatFromName(formatName);
      return std::make_shared<T>(format);
    });

    for (auto ext : exts) {
      sm.addExtensionBinding(ext, [formatName]() -> std::shared_ptr<GTBScanner> {
        auto format = Format::formatFromName(formatName);
        return std::make_shared<T>(format);
      });
    }
