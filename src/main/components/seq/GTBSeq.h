...
class GTBSeq : public GTBFile {
 public:
  GTBSeq(const std::string &format, RawFile *file, uint32_t offset, uint32_t length = 0,
  ...
  virtual void useColl(const GTBColl* coll) {}
  virtual MidiFile *convertToMidi(const GTBColl* coll = nullptr);
  ...
  bool saveAsMidi(const std::string &filepath, const GTBColl* coll = nullptr);
