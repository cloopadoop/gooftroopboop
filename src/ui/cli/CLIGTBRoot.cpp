...
void CLIGTBRoot::displayUsage() {
  cerr << "Usage: " << CLI_APP_NAME << " input_file1 input_file2 ... -o output_directory" << endl;
}

// displays a help message
void CLIGTBRoot::displayHelp() {
  cerr << "Goof Troop Boop version " << GTB_VERSION << endl;
  cerr << "A Goof Troop music editor based on VGMTrans." << endl << endl;
  displayUsage();
}

// creates the output directory if it does not already exist
bool CLIGTBRoot::makeOutputDir() {
...
bool CLIGTBRoot::openRawFile(const string &filename) {
  string fname = filename;
  cout << "Loading " << fname << endl;
  return GTBRoot::openRawFile(filename);
}

bool CLIGTBRoot::init() {
  if (inputFiles.empty()) {
    ...
    for(auto collNameIt = collNameMap.begin(); collNameIt != collNameMap.end(); ++collNameIt) {
      ...
      if (pairs.size() > 1) {  // name conflict
        ...
        for(pair<size_t, fs::path> p : pairs) {
          ...
          GTBColls()[p.first]->setName(newCollName);
        }
      }
    }
...
bool CLIGTBRoot::exportAllCollections() {
  bool success = true;
  for (GTBColl* coll : GTBColls()) {
    string collName = coll->name();
...
bool CLIGTBRoot::exportCollection(GTBColl* coll) {
    string collName = coll->name();
    cout << "Exporting: " << collName << endl;
    return saveMidi(coll) & saveSF2(coll) & saveDLS(coll);
}

bool CLIGTBRoot::saveMidi(const GTBColl* coll) {
  if (coll->seq() != nullptr) {
    string collName = coll->name();
...
bool CLIGTBRoot::saveSF2(GTBColl* coll) {
  string collName = coll->name();
...
bool CLIGTBRoot::saveDLS(GTBColl* coll) {
  string collName = coll->name();
...
void CLIGTBRoot::UI_setRootPtr(GTBRoot** theRoot) {
  *theRoot = &cliroot;
}

void CLIGTBRoot::UI_log(LogItem* theLog) {
...
string CLIGTBRoot::UI_getSaveFilePath(const string& suggestedFilename, const string& extension) {
  fs::path savePath = outputDir / fs::path(ConvertToSafeFileName(suggestedFilename) + "." + extension);
  return savePath.string();
}

string CLIGTBRoot::UI_getSaveDirPath(const std::string& suggestedDir) {
